...
--        , Run Battery ["-t", "<left>%", "--", "-c", "energy_full"] 60
        , Run Com "mybat" [] "Bat" 100
        , Run Com "mytemp" [] "Temp" 10
        , Run StdinReader
        ],
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader% }{ %multicpu%   %memory%   %swap%   %eth0%  %wlan0%  <fc=#FFFFCC>%date% %Bat% %Temp%</fc>"
}
